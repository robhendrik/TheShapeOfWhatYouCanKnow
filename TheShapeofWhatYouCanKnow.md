...
> *Figure 5: For n = 4, there are 12,870 balanced deterministic encodings. Of these, 20 are majority-optimal. Those 20 collapse to 14 distinct points in advantage space (because different encodings can produce the same point), three classes under permutation symmetry, and finally two vertex types after discarding points that are not vertices.*
...
> *Figure 7: Now add the quantum resource. These are the same four cross-sections as in Figure 4. The complicated classical polyhedra are gone: for every cut teh reachable region is bounded by a sphere. Instead of making the strategy space more complicated, quantum mechanics makes its geometry remarkably simple.*
